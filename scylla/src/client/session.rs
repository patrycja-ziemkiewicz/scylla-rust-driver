...
pub struct SessionConfig {
    ...
    ///
    /// When set, the session is pinned to that one node. Mutually exclusive with
    /// [`known_nodes`](Self::known_nodes): a maintenance-mode session has exactly
    /// one endpoint, and it is this one.
...
impl Session {
    ...
    pub async fn connect(mut config: SessionConfig) -> Result<Self, NewSessionError> {
        // Decide where this session connects before anything else: the choice
        // between known nodes and a maintenance endpoint is made once, here, and
        // everything downstream reads it off the result.
