...
impl ClusterState {
    pub(crate) async fn wait_until_all_pools_are_initialized(&self) {
        // Deliberately not the ring: it only holds nodes that own tokens, so a
        // zero-token node - one still bootstrapping, or the single node of a
        // maintenance mode session - would be skipped and its pool never waited
        // for, letting the first request race the pool's first fill and fail with
