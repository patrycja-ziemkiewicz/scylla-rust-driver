...
pub(crate) enum UntranslatedEndpoint {
    ...
    ///
    /// Distinct from [`Self::ContactPoint`] because it is not a *starting* point
    /// for discovering a cluster - it is the entirety of it. Being this variant is
    /// what tells the control connection not to read the peer list, and it is
    /// never replaced by a fetched peer, so the control connection always
    /// re-establishes to the very endpoint the user named.
    Maintenance(MaintenanceEndpoint),
}
...
impl UntranslatedEndpoint {
    ...
    ///
    /// The control connection consults this to decide not to read the peer list.
    ...
    pub(crate) fn transport(&self) -> &Transport {
        match self {
            // Anything discovered through the cluster is reached over TCP; only a
            // maintenance mode session may be pointed at a Unix socket.
...
impl Metadata {
    ...
    pub(crate) fn new_dummy(initial_peers: &[UntranslatedEndpoint]) -> Self {
        // A maintenance mode session has one endpoint and no peer list to invent
        // a stand-in for; the endpoint itself is the whole topology.
